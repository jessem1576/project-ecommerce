...
// Helper component to manage client-side state for variant selection
function ProductVariantSelector({ product, reviews }: { product: Product, reviews: Review[] }) {
                    ...
                    {uniqueColors.map(color => (
                      ...
                      >
                        {product.variants.find(v => v.color === color)?.image ? (
                          <Image src={product.variants.find(v => v.color === color)!.image!} alt={color} width={20} height={20} className="mr-2 rounded-sm"/>
                        ) : null}
                        {color}
                      </Button>
                    ))}
